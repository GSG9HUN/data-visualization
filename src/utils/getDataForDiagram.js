...
import {NAPopulation,EUPopulation,JPPopulation} from "./population"
const million = 1000000
export function getDataForDiagram(data, isMulti = false, year = "", type = "", isDependingPop = false) {
    if (year === "----" && type === "----" &&!isDependingPop) {
        return data
    }
    let dataForDiagrams;
    ...
    if (isMulti !== isDependingPop) {
        dataForDiagrams = createDataForMultiLineDiagram(dataForDiagrams, type)
    }
    if (isDependingPop) {
        let EU = createDataForMultiLineDiagramDependingOnPop(data, "Genre", "EU_Sales",EUPopulation)
        let NA = createDataForMultiLineDiagramDependingOnPop(data, "Genre", "NA_Sales",NAPopulation)
        let JP = createDataForMultiLineDiagramDependingOnPop(data, "Genre", "JP_Sales",JPPopulation)
        dataForDiagrams = {"EU":EU,"NA":NA,"JP":JP}
    }
...
function createDataForMultiLineDiagramDependingOnPop(data, type,region,population) {
    let lines = new Map();
    let dataForMultiLineDiagram = []
    data.map((x) => {
        if (lines.get(x[type])) {
            lines.get(x[type]).push({xValue: x.Year, yValue: (x[region]*million*100)/population[x.Year]})
        } else {
            lines.set(x[type], [{xValue: x.Year, yValue:(x[region]*million*100)/population[x.Year]}]);
        }
    })
